...
#   4. honest exit code: launchd sees the truth (alerting lives in run_nightly.py)
#   5. caffeinate hold: the Mac cannot sleep mid-pull
#
# SIXTH defence, specific to this worker: it will not start while zomato-ingest is
# running. Both drive Playwright against the SAME saved session file, and
# zomato-ingest writes the session back on exit. Two concurrent runs could clobber
# each other's cookies and cost a manual re-bootstrap.
#
# run_nightly.py exits 75 to DEFER (reports not arrived yet, or a transport blip).
# A deferral writes no stamp and raises no alert; the next slot simply tries again.
#
# Must live on the LOCAL disk (launchd cannot execute from iCloud Drive).
# Pass --force to run even if tonight already succeeded.
set -o pipefail
...
# Defence 6: never run alongside the sibling worker, they share a browser session.
if pgrep -f "zomato-ingest/run_evening.py" >/dev/null 2>&1; then
  log "===== slot at $(date): skipped, zomato-ingest is running (shared session) ====="
  exit 75
fi
...
log "===== zomato-business slot at $(date) ====="
