-- ============================================================
-- Migration 193: never show a rejection without its items (26 Aug 2026)
--
-- Pranjay, looking at a live store page: "when you show that one is rejected
-- by the store and you show the items are out of stock, but you don't show
-- which item was ordered, it is very difficult to understand."
--
-- He is right, and the cause is F31. Zomato's business ITEM export has
-- produced nothing since 23 Aug 2026, so on 24, 25 and 26 Aug every basket on
-- every page rendered as a dash: rejections, complaints, low ratings and
-- false-ready orders all listed with no item detail at all. A rejection reason
-- with no item is not actionable.
--
-- The items are not lost. The EVENING order-history feed
-- (landing.zomato_order_details, the 18:00 pull) still carries clean item text
-- in the same shape. Verified before writing this: 100% of orders populated on
-- every day from 22 to 26 Aug, 2,302 of 2,318 orders on 26 Aug joinable by
-- order id (99.3%), and the feed's own date sits on the SAME business day as
-- the order table on all 2,302 (offset 0), so no date shifting is needed.
--
-- So each of the three basket-building functions now falls back to that feed
-- when the item export has nothing for an order. It stays a FALLBACK and not
-- the primary because it carries item names and quantities only: no catalogue
-- id, category or unit cost, which the item export gives when it works. When
-- Zomato fixes their export the primary takes over again on its own, with no
-- further change here.
--
-- Built from the LIVE definitions as they stood after migrations 191 and 192,
-- so the corrected rejection vocabulary and the shut-shop tracker are kept.
-- ============================================================
