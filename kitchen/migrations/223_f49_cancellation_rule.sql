-- ============================================================
-- Migration 223: F49, classify a cancellation by what happened, not by Zomato's label
-- (9 September 2026, Pranjay's go: "Go ahead with the F49 page change")
--
-- The defect (integration-notes F49, confirmed by three sources): Zomato's
-- business report labels a post-pickup customer cancellation "Unavailable to
-- accept the order". Petpooja records the full sentence, "Customer
-- cancellation because they are unavailable to accept the order", and the
-- order timeline shows the store accepted, made and handed over the food.
-- Migration 191 put that label in the store-caused list, so those orders
-- printed under "Turned away or cancelled on the store" with the full bill
-- as money lost, and the shut-shop tracker counted them as "shop was shut".
--
-- The rule now:
--   store-caused rejection   rejected_by = 'Mx rejected'   (Zomato's own flag;
--                            covers Items out of stock, Kitchen is full,
--                            Restaurant is closed, Timeout, Merchant device
--                            issue, Wrong restaurant address)
--   shut shop                Restaurant is closed AND rejected_by = 'Mx rejected'
--   returned                 rejected_by = 'Cancelled' AND picked_up_at set:
--                            cancelled after the rider took it. Own list on
--                            every page; value lost = subtotal minus Zomato's
--                            compensation_for_customer_cancellation; NOT in
--                            the avoidable-loss total.
--   other cancellations      everything else (customer, rider), counted only.
--
-- Derived mechanically from the live definitions in 195 with the changes
-- above and nothing else; every replacement was asserted to hit the expected
-- number of places. Measured effect is recorded in integration-notes 14e.
-- ============================================================
...
ow as (
  select restaurant_id,
    ...
    count(*) filter (where order_state = 'Delivered'
      ...
      and extract(epoch from (left(nullif(picked_up_at,'NA'),19)::timestamp
        - left(nullif(rider_reached_outlet_at,'NA'),19)::timestamp))/60.0 >= 3) as fr,
    round(coalesce(sum(order_subtotal::numeric) filter (where rejected_by = 'Mx rejected'),0)) as stockout,
...
rej as (
  ...
  from ord where rejected_by = 'Mx rejected'
),
comp as (
...
ret as (
  select d, dlabel, tm, rejection_reason, basket, round(subtotal) as value,
    round(comp) as comp, round(greatest(subtotal - comp, 0)) as net, placed_at
...
select jsonb_build_object(
  ...
  'returned_day', (select coalesce(jsonb_agg(jsonb_build_object(
      'time', tm, 'reason', rejection_reason, 'basket', basket, 'value', value, 'comp', comp, 'net', net)
      order by placed_at), '[]'::jsonb) from ret where d = p_date),
  'returned_wk', (select coalesce(jsonb_agg(jsonb_build_object(
      'dlabel', dlabel, 'time', tm, 'reason', rejection_reason, 'basket', basket, 'value', value, 'comp', comp, 'net', net)
...
money_store as (
  select code,
    coalesce(sum(subtotal) filter (where rejected_by = 'Mx rejected'),0) as stockout_wk,
    ...
    count(*) filter (where rejected_by = 'Mx rejected') as rej_wk,
...
select jsonb_build_object(
  ...
  'returned', (select coalesce(jsonb_agg(jsonb_build_object(
      'code', code, 'am', p_am, 'dlabel', dlabel, 'time', tm, 'reason', rejection_reason,
  ...
  'shut_orders', (select coalesce(jsonb_agg(jsonb_build_object(
    ...
    from ord r where r.rejection_reason = 'Restaurant is closed' and r.rejected_by = 'Mx rejected'),
  'shut_stores', (select coalesce(jsonb_agg(x order by (x->>'orders')::int desc,
      ...
      from ord where rejection_reason = 'Restaurant is closed' and rejected_by = 'Mx rejected'
      group by code) s),
