-- ============================================================
-- Migration 225: F49 rule correction, applied minutes after 223 (9 Sep 2026)
--
-- 223 tested "rejected_by = 'Mx rejected'" alone. Zomato's business export
-- writes 'Mx rejected' as a DEFAULT on every delivered order too (16,094 of
-- 16,380 orders in the week to 6 Sep), so 223 counted nearly every delivered
-- order as a store rejection for the minutes it was live. The store-caused
-- rule is now: order_state <> 'Delivered' AND rejected_by = 'Mx rejected'.
-- Nothing else changes from 223. Smoke-tested against the week to 6 Sep:
-- store-caused rejections must be about 62, not 16,156.
-- ============================================================
...
ow as (
  select restaurant_id,
    ...
    count(*) filter (where order_state = 'Delivered'
      ...
      and extract(epoch from (left(nullif(picked_up_at,'NA'),19)::timestamp
        - left(nullif(rider_reached_outlet_at,'NA'),19)::timestamp))/60.0 >= 3) as fr,
    round(coalesce(sum(order_subtotal::numeric) filter (where (order_state <> 'Delivered' and rejected_by = 'Mx rejected')),0)) as stockout,
