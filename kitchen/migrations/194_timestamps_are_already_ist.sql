-- ============================================================
-- Migration 194: every displayed time was 5 hours 30 minutes late (28 Aug 2026)
--
-- Found by area manager Ajay Rana, who knows what time his stores trade:
-- "Our stores close at 2:00 AM. Janakpuri on 22nd, an order was received at
-- 1:54 AM and was prepared on time, but the rider was assigned only at
-- 2:10 AM." Our page showed that order at 07:24 am.
--
-- He is right. The order feed stores IST wearing a fake "+0000 UTC" label:
-- the raw value for that order is "2026-08-22 01:54:29 +0000 UTC" and 01:54
-- IST is exactly when it arrived. Migration 150 misread the label, added
-- 5h30 to reach "IST", and every function since inherited it. So every clock
-- time on every page and every mail has been 5 hours 30 minutes late since
-- the daily module shipped.
--
-- The proof that settles it, orders per hour across 20 to 26 Aug 2026:
--
--   hour   raw (no shift)   with our +5h30
--   02:00        2              1,868
--   03:00        0              2,122   <- the busiest hour, at 3am
--   04:00        0              2,017
--   08:00      139                  0
--   21:00    2,055                956
--   22:00    2,050                963
--
-- Raw gives a cake shop: a long afternoon climb, a 9 to 11pm peak, and NOTHING
-- between 2am and 6am because the stores shut at 2am. Our shifted version puts
-- the busiest hour at 3am and no orders at all through the morning. There is no
-- reading of that table where the shift is right.
--
-- WHAT THIS DOES NOT TOUCH. Rider wait and food prep are differences between
-- two timestamps, so the shift cancelled out and those numbers were always
-- right. Business dates come from their own column, so day assignment, all
-- counts, all money and all rankings were right too. This is display only,
-- but it is on every receipt on every page.
--
-- CONSEQUENCE WORTH RE-READING: the shut-shop tracker's hour breakdown now
-- reads completely differently. What looked like a 07:00 cluster ("the listing
-- goes live before the shop does") is really 01:30, and the 03:00 to 05:00
-- cluster is really 21:30 to 23:30. The real pattern is orders arriving AT AND
-- AFTER the 2am close, which is a Zomato listing-hours problem, not store
-- indiscipline. Ajay said as much in the same message.
-- ============================================================
...
items_fb as (
  ...
  select t.zomato_order_id, t.items_in_order as basket
  from landing.zomato_order_details t, ws
  where t.superseded_by is null
    and t.order_date between (ws.s - 1) and (ws.e + 1)
    and nullif(t.items_in_order, '') is not null
    and t.items_in_order not like '[0x%'
),
ord as (
  select b.business_date::date as d, b.zomato_order_id, b.order_state, coalesce(it.basket, fb.basket) as basket,
...
ord as (
  select o.code, b.business_date::date as d, b.zomato_order_id, b.order_state, coalesce(it.basket, fb.basket) as basket,
...
ord as (
  select o.code, o.am, b.business_date::date as d, b.order_state, coalesce(it.basket, fb.basket) as basket,
    -- the day label is the BUSINESS date, never the wall clock: an order placed
