-- ============================================================
-- CORRECTION carried in this migration (found 26 Aug 2026): the store-caused
-- rejection reasons in migration 180 were taken from the QUALITY report's
-- column names ('Outlet closed', 'Device issues'). The ORDER feed uses
-- different words and neither string ever matches a row, while its own
-- 'Restaurant is closed' and 'Unavailable to accept the order' were missed.
-- In the 7 days to 15 Aug 2026 that is 13 rejections and their value left out
-- of the area page's list and its money-lost line. The list below is the
-- ORDER feed's own vocabulary, checked against every distinct value present.
-- Migration 180 needs the same one-line correction.
-- ============================================================
-- Migration 190: dash_central_detail (central / network page, 26 Aug 2026)
--
-- The network-level twin of migration 180's dash_area_detail. The central
-- page asks a third question: not "what happened here" (store) and not
-- "which of my stores needs me today" (area), but "where do I put pressure
-- across the network, and which levers do I pull". So:
--   * every receipt names its OUTLET and its AREA MANAGER, because central
--     acts through the AM, never directly on the store;
--   * the same operational sections the area page carries, unfiltered;
--   * plus the levers only central sees (discounts, ad spend, funnel),
--     broken down per store so a lever number also lists what is behind it;
--   * plus a 7-day network trend so the charts carry real series.
-- One call returns everything; the page never queries per store.
--
-- Additive and read-only. Definitions reuse dash_all / dash_area_detail
-- exactly (F25 segment filter, kitchen prep time excluded, rider wait as the
-- verified speed measure, false ready = ready within 1 min and rider then
-- waited 3+ min) so the three pages can never disagree.
-- ============================================================
