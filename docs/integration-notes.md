...
Two findings from that first real run are recorded in `zomato-order-details-feed.md` sections 4d and 6, and both change earlier assumptions:
1. **The scraper cannot run headless.** Zomato's edge refuses headless browsers outright, so the evening slots open a visible Chromium window. That inherits F14's dependency on the Mac being awake and logged in.
...
3. **The trailing-window question is still open, and the first answer was an artefact.** Run 138 suggested days 3 to 8 old still change ~20% each, but it was comparing CSV pulls against the xlsx backfill; run 140, the first same-shape comparison, found ZERO changes on days 2 to 7. So the 20% was export-shape noise (same family as F19/KPT), and Pranjay's 3-day instinct now looks right. The window stays at 7 until a week of evening-to-evening (24h apart) pulls confirms it, since run 140 sat only 2 hours after 138 and ratings/complaints/refunds arrive over days. Cost of waiting is nil: a 7-day window is one export either way.
